...
void ExceptionHandler(ExceptionType which)
{
	...
	case SyscallException:
	{
		...
		case SC_Open:
			...
			{
				if (type == 0 || type == 1)
				{ // trường hợp 0: read and write và 1: read only
		...
		case SC_Read:
			...
			if (idRead < 2 || idRead > MAX_FILE - 1)
			{ // File không nằm trong khoảng 20 file discriptor
				kernel->machine->WriteRegister(2, -1);
				IncreasePC();
				break;
			}

			if (kernel->fileSystem->of[idRead] == NULL)
			{ // File không tồn tại
				kernel->machine->WriteRegister(2, -1);
				IncreasePC();
				break;
			...
			if (kernel->fileSystem->of[idRead]->type == 3)
			{ // Console OUT
				kernel->machine->WriteRegister(2, -1);
				IncreasePC();
				break;
			...
			if (kernel->fileSystem->of[idRead]->type == 2)
			{ // Console IN
				char chRead;
				int sizeRead;
				sizeRead = 0;
		...
		case SC_Write:
			...
			if (kernel->fileSystem->of[idWrite]->type == 0)
			{ // Xử lý trường hợp Read and write
			...
			if (kernel->fileSystem->of[idWrite]->type == 3)
			{ // Xử lý trường hợp console output
				int i = 0;
