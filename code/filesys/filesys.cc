...
{ 
    ...
    delete directory;
    index++;
    return of[index - 1]; // return NULL if not found
}
...
OpenFile* FileSystem::Open(char *name, int type)
{
	...
	sector = directory->Find(name);
	if (sector >= 0)
		of[slot] = new OpenFile(sector, type);	// name was found in directory 
	delete directory;
	return of[slot];				// return NULL if not found
}

int FileSystem::CheckSlot()
{
	for(int i = 2; i < MAX_FILE; i++)
	{
		if(of[i] == NULL) return i;		
	}
	return -1;
}
